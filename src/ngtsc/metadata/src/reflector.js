...
function reflectConstructorParameters(node, checker) {
    ...
    // clang-format off
    const maybeCtor = node
        .members
        .filter(element => ts.isConstructorDeclaration(element));
    // clang-format on
...
function reflectDecorator(decorator, checker) {
    ...
    let decoratorOfInterest = decorator.expression;
    let args = [];
    ...
    return Object.assign({}, importDecl, { node: decorator, args });
}
